  ...
  function getShape(geometry) {
    ...
    switch (geometry.type) {
      ...
      case 'CylinderBufferGeometry':
				...
				const c = new CANNON.Cylinder(radiusTop, radiusBottom, height, radialSegments);
				return c;
  ...
  function setMeshPosition(mesh, position, index = 0) {
    ...
    if (mesh.isInstancedMesh) {
      ...
      var bodies = meshMap.get(mesh);
			...
			// bodies[index].linearDamping = 1;
			// bodies[index].angularVelocity = new CANNON.Vec3(1, 1, 0);
			// bodies[index].applyLocalImpulse(new CANNON.Vec3(-1, -1, -1), new CANNON.Vec3(0.6, 0.7, 0.6));
